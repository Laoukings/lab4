...
void DSets::join(int r, int s) {
    ...
    if (V[s] < V[r]) {
        V[s] += V[r];// update size of s's tree
        V[r] = s;// r now points to s (s is new root)
    }
    else {
        V[r] += V[s];// update size of r's tree
        V[s] = r;// s now points to r (r is new root)
    }
}
...
int DSets::find(int x) {
    ...
    // path compression, also updates the parent of each element along the path to point directly to the root. 
    if (V[x] < 0) {
        return x;
    } else {
        return V[x] = find(V[x]); //Path compression: update V[x] to point directly to the root
    }
